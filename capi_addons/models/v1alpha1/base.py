...
class LifecycleHook(schema.BaseModel):
    ...
    api_version: schema.constr(pattern=r"^([a-z][a-z0-9\.-]*/)?v[0-9][a-z0-9]*$") = (
        Field(..., description="The API version of the resource(s) to operate on.")
    )
    ...
    options: schema.Dict[str, schema.Any] = Field(
        default_factory=dict, description="Options for the action."
    )
    ...
    async def _apply_patch(self, ek_resource, addon, patch_type, patch_data):
        ...
        if self.name:
            try:
                await patch_func(
                    self.name, patch_data, namespace=addon.spec.target_namespace
                )
            ...
            async for resource in ek_resource.list(
                labels=self.selector, namespace=addon.spec.target_namespace
            ):
                await patch_func(
    ...
    async def _patch(self, ek_resource, addon):
        ...
        await self._apply_patch(
            ek_resource, addon, self.options.get("type", "strategic"), patch_data
        )
    ...
    async def _restart(self, ek_resource, addon):
        if self.api_version != "apps/v1" or self.kind not in {
            "Deployment",
            "DaemonSet",
            "StatefulSet",
        }:
...
class AddonSpec(schema.BaseModel):
    ...
    lifecycle_hooks: LifecycleHooks = Field(
        default_factory=LifecycleHooks, description="Lifecycle hooks for the addon."
    )
    paused: bool = Field(
        False,
        description=(
...
class AddonResource(schema.BaseModel):
    ...
    namespace: str = Field("", description="The namespace of the resource, if present.")
...
class AddonStatus(schema.BaseModel, extra="allow"):
    ...
    notes: schema.Optional[str] = Field(None, description="The notes from the release.")
...
class Addon(CustomResource, abstract=True):
    ...
    spec: AddonSpec = Field(..., description="The spec for the addon.")
    status: AddonStatus = Field(
        default_factory=AddonStatus, description="The status for the addon."
    )
    ...
    async def init_metadata(
        ...
        if (
            ...
            and cloud_identity["metadata"]["namespace"] == self.metadata.namespace
        ):
    ...
    async def _get_current_revision(
        self, ek_client: AsyncClient, helm_client: Client
    ) -> ReleaseRevision:
        """
        ...
        try:
            ...
            if current_revision.status in {
                ...
                await current_revision.release.uninstall(
                    timeout=self.spec.release_timeout, wait=True
                )
                return None
                ...
                return await current_revision.release.rollback(
                    cleanup_on_fail=True, timeout=self.spec.release_timeout, wait=True
                )
            else:
    ...
    async def install_or_upgrade(
        ...
        async with chart_context as chart:
            ...
            if should_install_or_upgrade:
                self.set_phase(
                    AddonPhase.UPGRADING if current_revision else AddonPhase.INSTALLING
                )
        ...
        if (
            current_revision.status == ReleaseRevisionStatus.DEPLOYED
            and self.status.phase != AddonPhase.DEPLOYED
        ):
    ...
    async def get_chart(
        ...
        with tempfile.TemporaryDirectory() as chart_directory:
            ...
            async for resource in self.get_resources(
                template_loader, ek_client, cluster, infra_cluster, cloud_identity
            ):
